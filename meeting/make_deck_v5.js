...
// ═══════════════ 1. 표지 ═══════════════
{
  ...
  const cards = [
    ...
    ["원인을 특정했다", "해악은 '질문 → 무관이미지' 읽기 경로\n하나로만 흐른다. 이미지 간 혼합을 끊으면\n오히려 −0.29로 크게 나빠진다", "읽기차단 ≡ 전면차단, CI [0, 0]", "7EC8F5"],
...
// ═══════════════ 4. 경로 분해 (핵심) ═══════════════
{
  ...
  box(0.55, 6.05, "① 읽기 경로 차단 = 전면 차단, 소수점까지 동일",
      "차이 0.0000, 95% CI [0, 0].\n방해 이미지의 영향은 '질문 토큰이 그것을 읽는'\n경로 하나로만 흐릅니다. 다른 채널이 없습니다.",
      C.lightgreen, C.green, C.green);
  box(6.78, 6.02, "② 이미지 간 혼합을 끊으면 오히려 크게 나빠짐",
      "3B −0.294 · 7B −0.422.\n이미지끼리 보는 것은 도움이 됩니다.\n설계 가설과 정반대 방향입니다.",
  ...
  foot(s, "n=300 · AUC · 4D mask의 (질의 행 × 키 열) 사각형 단위 개입. 읽기차단 ≡ 전면차단은 구현 정확성 검증도 겸함");
  takeaway(s, "→ 개입해야 할 곳은 '이미지를 서로 떼어놓는 것'이 아니라 '읽기 경로를 제어하는 것'입니다");
  s.addNotes("이 슬라이드가 오늘의 핵심입니다. 인과 순서상 이미지1은 이미지2를 볼 수 없으므로 경로는 둘뿐입니다. 놀랍게도 질문이 방해 이미지를 읽는 경로만 끊으면 전면 차단과 결과가 소수점까지 같습니다. 반대로 이미지끼리 보는 경로를 끊으면 크게 나빠집니다. 즉 저희가 처음에 세웠던 '정보가 섞여서 나쁘다'는 가설은 틀렸고, 섞이는 것은 오히려 도움이 됩니다.");
}
...
// ═══════════════ 7. 컨트롤러 ═══════════════
{
  ...
  s.addTable([
    [m("방해 수", hd), m("컨트롤러 정확도", hd), m("무작위 기준", hd), m("손익분기", hd),
     m("개입 없음", hd), m("실현 성능", hd)],
    [m("1", { bold: true }), m("0.693"), m("0.500"), m("0.761", { color: C.red }),
     m("0.6978"), m("0.6669  (−3.1%p)", { color: C.red })],
    [m("2", { bold: true }), m("0.707"), m("0.333"), m("0.661"),
     m("0.6063"), m("0.7126  (+10.6%p)", { bold: true, color: C.green })],
    [m("3", { bold: true }), m("0.730", { bold: true }), m("0.250"),
     m("0.569", { bold: true, color: C.green }), m("0.5213"),
     m("0.7028  (+18.2%p)", { bold: true, color: C.green })],
  ], { x: 0.55, y: 3.05, w: 12.25, rowH: [0.45, 0.42, 0.42, 0.46], ...TBL });

  s.addText([
    { text: "두 곡선이 반대로 움직입니다.  ", options: { bold: true, fontSize: 12.5, color: C.navy } },
    { text: "컨트롤러 정확도는 방해가 늘수록 올라가고(0.693 → 0.730), 손익분기는 내려갑니다(0.761 → 0.569). 손상이 클수록 오판 비용이 상대적으로 싸지고, 동시에 반사실 신호가 강해져 판정도 정확해지기 때문입니다.", options: { fontSize: 12.5, color: C.navy } },
  ], { x: 0.55, y: 4.95, w: 12.25, h: 0.8, fontFace: F, margin: 0 });
  foot(s, "3B n=300. 컨트롤러 정확도와 손익분기는 정확한 값이며, '실현 성능'은 오판 시 조건을 근사한 낙관값 — 0.730 > 0.569 판정은 근사에 의존하지 않음. forward N+1회 필요(정확도 트랙)");
  takeaway(s, "→ 무관 입력이 많을수록 방법이 더 잘 작동합니다. 방법이 필요한 상황이 곧 방법이 통하는 상황입니다", C.green);
