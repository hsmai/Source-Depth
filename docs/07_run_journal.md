...
## 2026-08-13 — 지표가 바뀌자 결론이 바뀐 날

### 죽은 줄 알았던 결과 회수
위치 균형화(posbal)가 3B·7B **각각 4200/4200 완료**되어 있었다 (전날 로그 없이 중단된 줄 알았음).
최종: 배분 이득이 원순서 → 뒤집기에서 3B **+28.7 → +24.8%p**, 7B **+8.7 → +7.5%p**.
**두 스케일 모두 유지율 87%.** 중간집계(275문항, 80%)보다 좋아졌고 n=600에서 확정.
덱에 박혀 있던 +30.7%p를 세 군데 모두 정정.

### 99327 — 다중 distractor (사전등록 `docs/14`)
사전에 검정을 **McNemar → Fisher 정확검정**으로 정정 (leak/ctrl이 독립 문항집합임을
부분 결과에서 확인, 임계값은 불변).

| | ctrl | leak | 오염 | p | 기준선(1장) |
|---|---|---|---|---|---|
| **7B** | 0.9867 | 0.9600 | **+2.7%p** | 0.282 | +2.0%p |
| **3B** | 0.9733 | **0.5533** | **+42.0%p** | 1.6e-19 | +26.0%p |

**주 종점 실패** (7B). 부 종점 통과 (3B, N 단조성 +16.0%p). 3B는 T0에서 0.553 → **0.913** 회복.

### 사후 발견 — 우리 지표가 한쪽 방향만 보고 있었다 (`docs/15`)
| 방해 이미지 추가 | 3B | 7B |
|---|---|---|
| 실패 방향 | **누출**(yes 오답) | **과잉 억제**(no 오답) |
| 셀1(gt=no) | −17.3%p | +2.0%p (안 보임) |
| 셀2·4(gt=yes) | −10.7%p | **−8.0%p** (p<.005) |
| **ΔAUC(M−S)** | **−0.0698** | **−0.0269** (CI 0 배제) |
| ΔAUC(T0−M) 회복 | **94%** | **92%** |

held-out(위치 뒤집기)에서 재현: 3B +0.0380 [+.019,+.057], 7B +0.0183 [+.007,+.030].
**2 모델 × 2 순서 = 4개 조건 전부 유의.** → "7B에서 현상이 사라진다"는 **지표의 오류였다.**

### 99345 실패 → 99346 재제출 (scale-AUC, 사전등록 `docs/16`)
PBS가 배정한 GPU가 A6000이 아닌 **RTX 3090(24GB)**. 7B 가중치 16.5GB 위 여유 7GB.
스크립트가 N=1·2·3 입력을 동시 보유 → 문항당 이미지 9장 분량 → 4번째 문항에서 OOM.
N별 순차 처리로 피크를 4장으로 축소, OOM 1회 재시도, `expandable_segments` 추가.
99346 정상 진행 중.

### 준비 완료 (GPU 대기)
`19_controller_norope.py` — 컨트롤러를 **RoPE 제거 attention**으로 재측정.
우리 사망 진단이 선행연구가 "고장났다"고 지목한 기준 위에서 내려졌을 가능성.
판정 기준 사전 고정: **16층 이하 위치무관 ≥ 0.80이면 사망 진단 철회.**
